...
namespace client_api_test_service_dotnet
{
    ...
    public class ApiVerifierController : ApiBaseVCController
    {
        ...
        public ApiVerifierController(IConfiguration configuration, IOptions<AppSettingsModel> appSettings,
            IMemoryCache memoryCache, IWebHostEnvironment env, ILogger<ApiVerifierController> log) : base(configuration,
            appSettings, memoryCache, env, log)
        {
        ...
        protected JObject GetPresentationRequest()
        {
            ...
            presentationRequestFile =
                presentationRequestFile.Replace("%cd%", System.IO.Directory.GetCurrentDirectory());
            ...
            if (!HttpGet(config["presentation"]["requestedCredentials"][0]["manifest"].ToString(), out statusCode,
                out contents))
            {
                _log.LogError("HttpStatus {0} fetching manifest {1}", statusCode,
                    config["presentation"]["requestedCredentials"][0]["manifest"].ToString());
                return null;
            }
        ...
        public async Task<ActionResult> presentationCallback()
        {
            ...
            try
            {
                ...
                if (code == "presentation_verified")
                {
                    ...
                    // we are just sending the claims for now, not real VCs
                    // also we are adding the connector did just as credential
                    var did = vcClaims["identity-hub"]?.ToString();
                    if (!string.IsNullOrEmpty(did))
                        await SendCredential(did, vcClaims);
                    else
                        await SendCredential(vcClaims);
        ...
        private static async Task SendCredential(string did, JObject vcClaims)
        {
            try
            {
                var didResolver = new DidResolver();
                var serviceUri = await didResolver.GetIdentityHubEndpoint(did);
                if (serviceUri == null) return;

                var credential = new NameCredential
                {
                    FirstName = vcClaims["firstName"]?.ToString(),
                    LastName = vcClaims["lastName"]?.ToString()
                };

                var identityHubService = new IdentityHubService();
                var response = await identityHubService.SendToIdentityHubAsync(serviceUri, credential);
                var responseString = await response.Content.ReadAsStringAsync();

                Console.WriteLine($"Sent credential to {did}. Response {responseString}");
            }
            catch (Exception e)
            {
                Console.WriteLine($"Could not send credential to identity hub. {e}");
            }
        }

        private static async Task SendCredential(JObject vcClaims)
        {
            try
            {
                var credential = new NameCredential
                ...
                var identityHubService = new IdentityHubService();
                var response = await identityHubService.SendToConsumerAsync(credential);
                var responseString = await response.Content.ReadAsStringAsync();

                Console.WriteLine($"Sent credential to Consumer. Response {responseString}");
            }
            catch (Exception e)
            {
                Console.WriteLine($"Could not send credential to identity hub. {e}");
        ...
        public async Task<ActionResult> presentationResponse()
        {
            ...
            try
            {
                ...
                {
                    _log.LogTrace(
                        $"status={cacheData["status"].ToString()}, message={cacheData["message"].ToString()}");
